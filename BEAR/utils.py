...
class ReplayBuffer(object):
    ...
    def load(self, filename, bootstrap_dim=None):
        ...
        num_samples = np.array(self.storage['observations']).shape[0]
        if bootstrap_dim is not None:
            ...
            if self.added_to_storage:
                self.storage['bootstrap_mask'] = np.concatenate((self.storage['bootstrap_mask'],bootstrap_mask[:num_samples]), axis=0)
            else:
                self.storage['bootstrap_mask'] = bootstrap_mask[:num_samples]
        self.added_to_storage=True